<?php
    ...
    define( 'CASE_OK_IMAGE_RESIZED',                                   200 );
    define( 'CASE_OK_SMALLER_THAN_SMALLEST_BREAKPOINT',                201 );
    define( 'CASE_OK_IMAGE_AND_DEVICE_BIGGER_THAN_BIGGEST_BREAKPOINT', 202 );
    define( 'CASE_OK_CLIENT_WIDTH_DPI_BIGGER_THAN_BIGGEST_BREAKPOINT', 203 );
    define( 'CASE_OK_NO_COOKIE_OR_URL_PARAMETER',                      204 );
    
    define( 'CASE_ERROR_CACHE_DIRECTORY_NOT_READY', 401 );
    define( 'CASE_ERROR_RESIZED_IMAGE_NOT_CREATED', 402 );
        ...
        if ( $settings['debug'] == 'true' ) {
            ...
            <body>
                ...
                <table><tbody>
                    ...
                    <tr>
                        <td>Cache exists</td>
                        <td><?php echo file_exists( adaptive_images_script_get_main_cache_directory() ) ? 'YES' : 'NO'; ?></td>
                    </tr>
                    <tr>
                        <td>Cache is dir</td>
                        <td><?php echo is_dir( adaptive_images_script_get_main_cache_directory() ) ? 'YES' : 'NO'; ?></td>
                    </tr>
                    <tr>
                        <td>Cache writable</td>
                        <td><?php echo is_writable( adaptive_images_script_get_main_cache_directory() ) ? 'YES' : 'NO'; ?></td>
                    </tr>
                </tbody></table>
            ...
            // Debug was asked but not specified.

            adaptive_images_script_send_error_message( 'No debug command was given, sorry.' );
    ...
    function adaptive_images_script_get_main_cache_directory () {

        $settings = adaptive_images_script_get_settings();

        return $settings['wp_content'] . '/' . $settings['cache_dir'];
    ...
    /**
     * Sends an image file to the requesting browser.
     * 
     * @param string $filename      The filename of the image to send.
     * @param int    $browser_cache The browser cache expires time.
     * @param int    $case          Indicator to tell under which circumstances the image is sent.
     * 
     * @author Nevma (info@nevma.gr)
     * 
     * @return void
    ...
    function adaptive_images_script_send_image ( $filename, $browser_cache, $case = 0 ) {
        ...
        // If plugin browser cache is set to zero then do nothing, so that the default server settings will take effect.
        ...
        header( 'X-Adaptive-Images: O~o~. ' . $case . ' .~o~O' );
        readfile( $filename );
    ...
    /**
     * Checks if directory in the cache exists and is writeable and creates it if possible. Note that if the .htaccess 
     * file has not been updated correctly then the request for an image will never reach this script and then this 
     * code will not even be attempted to be called and the image will appear as "404 not found"/. Recall the starting 
     * slash ("/") issue in the Rewrite of the .htaccess file.
     * 
     * @author Nevma (info@nevma.gr)
     * 
     * @return void
    ...
    function adaptive_images_script_generate_image ( $source_file, $cache_file, $resolution, $jpg_quality, $sharpen ) {
        ...
        if ( $extension == 'jpg' || $extension == 'jpeg' ) {
            ...
            // Interpolates source image to destination image to make it clearer.
        ...
        // Check and ensure that cache directory to save the resized image is created and writeable.

        $cache_path = dirname( $cache_file );

        if ( ! adaptive_images_script_ensure_cache_directory_ready( $cache_path ) ) {

            return array( 'success' => false, 'message' => 'Cache directory for image not accessible or not writeable.' );
        ...
        switch ( $extension ) {
            ...
            case 'png':
                ...
                // Lower PNG compression, bigger file, less time to produce, higher PNG compression, smaller file, longer to produce.
            ...
            default:
                ...
                // High JPG quality, high image quality, bigger file, lower JPG quality, lower image quality, lower file size.
...
/**********************************************************************************************************************/
    ...
    // Output nothing, used for cases of standalone unit tests in the future.
    ...
    if ( $settings['debug'] ) {
        ...
        // Special case with debugging output to the browser.
    ...
    if ( ! file_exists( $settings['source_file'] ) ) {
        ...
        // If the original image does not exist then there is nothig to send.

        adaptive_images_script_send_error_message( 'Image was not found or not available, sorry.' );
        exit();
    ...
    if ( ! isset( $_GET['resolution'] ) && ! isset( $_COOKIE['resolution'] ) ) { 
        ...
        adaptive_images_script_send_image( $settings['source_file'], $settings['browser_cache'], CASE_OK_NO_COOKIE_OR_URL_PARAMETER );
        exit();
    ...
    // Ensure main cache directory is created and writeable.

    if ( ! adaptive_images_script_ensure_cache_directory_ready( adaptive_images_script_get_main_cache_directory() ) ) {

        // Send the original image itself as the best solution.

        adaptive_images_script_send_image( $settings['source_file'], $settings['browser_cache'], CASE_ERROR_CACHE_DIRECTORY_NOT_READY );
        exit();
    ...
    if ( $image_width > $settings['resolution'] && $settings['client_width'] > $settings['resolution'] ) { 
        ...
        adaptive_images_script_send_image( $settings['source_file'], $settings['browser_cache'], CASE_OK_IMAGE_AND_DEVICE_BIGGER_THAN_BIGGEST_BREAKPOINT );
        exit();
    ...
    if ( $settings['client_width'] * $settings['pixel_density'] > $settings['resolution'] ) {
        ...
        adaptive_images_script_send_image( $settings['source_file'], $settings['browser_cache'], CASE_OK_CLIENT_WIDTH_DPI_BIGGER_THAN_BIGGEST_BREAKPOINT );
        exit();
    ...
    if ( $image_width < $settings['resolution'] ) {
        ...
        adaptive_images_script_send_image( $settings['source_file'], $settings['browser_cache'], CASE_OK_SMALLER_THAN_SMALLEST_BREAKPOINT );
        exit();
    ...
    $cache_file = adaptive_images_script_get_main_cache_directory() . '/' . $settings['resolution'] . $settings['requested_uri'];
    ...
    if ( ! file_exists( $cache_file ) ) { 
        ...
        if ( ! $result['success'] ) {
            ...
            adaptive_images_script_send_image( $source_file, $settings['browser_cache'], CASE_ERROR_RESIZED_IMAGE_NOT_CREATED );
            exit();
    ...
    adaptive_images_script_send_image( $cache_file, $settings['browser_cache'], CASE_OK_IMAGE_RESIZED );
    exit();
